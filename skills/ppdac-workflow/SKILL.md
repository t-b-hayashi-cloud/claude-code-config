...
[Plan] ← 計画がない場合はここから
  analysis-planner（人間と協働）→ docs/analysis_plan/<分析名>.md 保存
       ...
       ↓ 修正が必要な場合: analysis-planner が修正 → adversary 再批評
...
[Conclusion] ← 分析が完了している場合はここから開始可能
  analysis-reporter → docs/analysis_result/<分析名>.md 保存
       ...
       ↓ 修正が必要な場合: analysis-reporter が修正 → adversary 再批評
...
**レビュワーは必ず並行起動する**（Agent tool で同時呼び出し）:

```python
# Phase 3/4 の品質ゲート（並行起動）
Agent(sql-reviewer), Agent(security-reviewer)  # 同時
Agent(code-reviewer), Agent(python-reviewer), Agent(security-reviewer)  # 同時

# Phase 1/2/5 の品質ゲート（単独起動）
Agent(adversary)  # 単独（レッドチーム批評）
```
